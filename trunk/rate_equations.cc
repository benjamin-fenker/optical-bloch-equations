...
Rate_Equations::Rate_Equations(Eigenvector_Helper set_eigen,
  ...
  totalTerms = eigen.atom.numFStates+eigen.atom.numGStates+eigen.atom.numEStates;
  ...
  */
  ...
  population = new double[eigen.atom.numFStates+eigen.atom.numGStates+eigen.atom.numEStates];
  int i, j;
...
void Rate_Equations::setup_transition_rates(double linewidth) {
  ...
  for (int q = 0; q < 3; q++) {
	...
	if (op_verbose) {
	  printf("|g=%d>-->|e=%d (%d)\t nu_eg = %14.10G MHz\tnu_L = %14.10G MHz\t",
		 g, e, q, transition_freq/_MHz, laser_ge.nu/_MHz);
	}
	...
	if (op_verbose) {
	  printf("|f=%d>-->|e=%d (%d)\t nu_eg = %14.10G MHz\tnu_L = %14.10G MHz\t",
		 f, e, q, transition_freq/_MHz, laser_fe.nu/_MHz);
	}
